...
### 📁 目录对比（Beyond Compare 式树形双栏）
- **树形组织**：默认仅显示顶层条目，目录可展开/折叠（双击或点 ▸），
  避免海量文件平铺难以定位；子项按「目录优先 + 自然字母序」排列
- **文件夹聚合状态**：文件夹显示子树聚合结果与差异计数徽标（`≠3 缺1 新2`），
  折叠状态下即可掌握内部差异
- 左右各有独立名称列：孤儿文件只出现在所属一侧（斜纹占位），按状态整行着色
- **中缝单行复制**：每行中间的 ◀/▶ 直接把该文件/文件夹复制到另一侧
  （文件夹为递归复制并保持 mtime，复制后立即显示为相同）
- 「只看不同」过滤（自动剪除全同子树）、展开/收起全部、快速 mtime / 精确 blake3 哈希
- 目录模式支持批量勾选左右复制、删除（带确认）；分支/多路模式同样按树组织（只读）
- 双击文件 → 切换到「文本对比」页签
